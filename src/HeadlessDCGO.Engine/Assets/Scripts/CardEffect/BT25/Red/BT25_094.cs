...
{
    public class BT25_094 : CEntity_Effect
    {
            ...
            if (timing == EffectTiming.OnAllyAttack)
            {
                bool PermanentCondition(Permanent permanent)
                {
                    return CardEffectCommons.IsPermanentExistsOnOwnerBattleAreaDigimon(permanent, card)
                        && (permanent.TopCard.CardColors.Contains(CardColor.Red) || permanent.TopCard.CardColors.Contains(CardColor.Blue))
            ...
            if (timing == EffectTiming.OptionSkill)
            {
                ...
                bool CanSelectCardCondition(CardSource cardSource)
                {
                    return cardSource.IsDigimon
                        && (cardSource.HasCardColor(CardColor.Red) || cardSource.HasCardColor(CardColor.Blue))

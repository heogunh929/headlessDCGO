// 1:1 headless mirror of the original BT1_094 (BT1/Red) — an Option.
...
//   maxCount:1, canEndNotMax:false — same shape as BT1_023's [On Play] Blocker-delete, ported to the
//   OptionSkill/SecuritySkill activation flow (same as ST1_15/ST1_16). CanTriggerOptionMainEffect /
//   HasMatchConditionPermanent + Min(1,count) are subsumed by the OptionSkill activation gate and
//   SelectAndDestroyEffect's own "select up to maxCount matching permanents" behaviour (no-op when nothing
//   matches). AS-IS permanent.HasBlocker is mirrored via the self-static keyword gate.
    ...
    public override IReadOnlyList<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OptionSkill)
        {
            ...
            cardEffects.Add(CardEffectFactory.SelectAndDestroyEffect(
                ...
                maxCount: 1,
                canEndNotMax: false,
                description: "[Main] Delete 1 of your opponent's Digimon with <Blocker>."));
        }

...
// NOTES (verbatim-missing / diverged substrate; see docs/audit/rebuild_p4_factory_missing.md — kept VERBATIM,
//   not stubbed/simplified, per the no-simplification directive):
//   - cardSource.Owner.CanIgnoreDigivolutionRequirement(permanent, cardSource): the mirror CardSource.Owner is a
//     HeadlessPlayerId value (no Player object / no CanIgnoreDigivolutionRequirement method).
//   - permanent.TopCard.CardColors.Contains(cardColor): the mirror CardColors is IReadOnlyList<string>, while the
//     AS-IS compares against a CardColor enum value.

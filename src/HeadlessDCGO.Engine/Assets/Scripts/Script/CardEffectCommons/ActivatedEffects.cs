...
// `RevealMultiSelectEffect` DELETED — census-0 among ported cards (BT2_044 / ST4_03 / ST4_10 call the
// AS-IS-signature commons `CardEffectCommons.SimplifiedRevealDeckTopCardsAndSelect` /
// `RevealDeckTopCardsAndProcessForAll` directly). The two classes were mirror-invented duplicates the commons
// reveal bridge (RevealLibrary.cs) is the strict superset of (that bridge was itself copied from these
// classes' verified StageMove/HandleRemaining logic). Their resolver cases + CardEffectFactory factory helpers
// are removed with them; the white-box witnesses (G9-016 / G9-029 / F1 reveal-remainder) are re-pointed to drive
// the commons directly, and the now-orphaned fixtures TfxRevealSelect / TfxSelectCardCond are retired.

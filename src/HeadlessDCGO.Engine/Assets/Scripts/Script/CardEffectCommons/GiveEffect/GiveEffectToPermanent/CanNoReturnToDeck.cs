...
// substrate `GainCanNotReturnToDeck` (CardEffectCommons.cs:3296). `cardEffectCondition` is adapted via the
...
{
    /// <summary>(BRIDGE) AS-IS <c>CardEffectCommons.GainCanNotReturnToDeck(...)</c>
    /// (GiveEffect/GiveEffectToPermanent/CanNoReturnToDeck.cs:10) — AS-IS-signature overload; delegates to the
    ...
    {
        GainCanNotReturnToDeck(targetPermanent, AdaptCardEffectCondition(cardEffectCondition), effectDuration, activateClass?.EffectSourceCard, effectName);

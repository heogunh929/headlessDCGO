...
// (J-4) 1:1 mirror of AS-IS CardEffectCommons.GainImmuneFromDPMinus (…/GiveEffectToPermanent/ImmuneFromDPMinus.cs
// :10-53): grant the TARGET permanent a timed "immune from DP-minus (by matching effects)" restriction. Builds the
// AS-IS kind-class via CardEffectFactory.ImmuneFromDPMinusStaticEffect (PermanentCondition = permanent==target,
// the caller's `cardEffectCondition` gating WHICH causing effects are ignored, live CanUseCondition =
// on-battle-area && !TopCard.CanNotBeAffected(cause)) and stores it in the target's duration bucket via
// AddEffectToPermanent(timing: EffectTiming.None). Read LIVE by Permanent.ImmuneFromDPMinus over EffectList(None)
// — the registry joint arm goes silent. The AS-IS coroutine only drove the CreateBuffEffect UI visual (dropped).
// The public AS-IS-signature `Task` overload threads the LIVE `activateClass` as the CanNotBeAffected cause and the
...
{
    /// <summary>1:1 mirror of AS-IS <c>GainImmuneFromDPMinus</c> (GiveEffect/GiveEffectToPermanent/ImmuneFromDPMinus.cs:10)
    ...
    private static bool GainImmuneFromDPMinusImpl(
        ...
        string effectName)
    {
        ...
        CardEffects.ImmuneFromDPMinusClass immuneFromDPMinusClass = CardEffectFactory.ImmuneFromDPMinusStaticEffect(  // AS-IS :34-40

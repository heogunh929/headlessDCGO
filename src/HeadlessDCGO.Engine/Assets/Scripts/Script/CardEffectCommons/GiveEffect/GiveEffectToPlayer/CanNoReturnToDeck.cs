...
// substrate `GainCanNotReturnToDeckPlayerEffect` (CardEffectCommons.cs:3312). `cardEffectCondition` is adapted
...
{
    /// <summary>(BRIDGE) AS-IS <c>CardEffectCommons.GainCanNotReturnToDeckPlayerEffect(...)</c>
    /// (GiveEffect/GiveEffectToPlayer/CanNoReturnToDeck.cs:10) — AS-IS-signature overload; delegates to the
    ...
    public static async Task GainCanNotReturnToDeckPlayerEffect(
        ...
        string effectName)
    {
        GainCanNotReturnToDeckPlayerEffect(permanentCondition, AdaptCardEffectCondition(cardEffectCondition), effectDuration, activateClass?.EffectSourceCard, effectName);

...
// substrate `GainImmuneFromDPMinusPlayerEffect` (CardEffectCommons.cs:3329). `cardEffectCondition` is adapted
...
{
    /// <summary>(BRIDGE) AS-IS <c>CardEffectCommons.GainImmuneFromDPMinusPlayerEffect(...)</c>
    /// (GiveEffect/GiveEffectToPlayer/ImmuneFromDPMinus.cs:10) — AS-IS-signature overload; delegates to the
    ...
    {
        GainImmuneFromDPMinusPlayerEffect(permanentCondition, AdaptCardEffectCondition(cardEffectCondition), effectDuration, activateClass?.EffectSourceCard, effectName);

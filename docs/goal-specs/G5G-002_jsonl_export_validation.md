# G5G-002 JSONL export validation 상세 지시서
...
- 우선순위: `중간`
- 선행 Goal: `G5G-001`
- 결과 문서: `docs/test-results/goals/G5G-002_jsonl_export_validation_unit_test_results.md`

## 2. 완성 목표

JSONL export validation Goal은 'JSONL export와 검증 구현'를 완성하기 위한 작업이다. transition, observation, action, reward, metadata를 JSONL 등 학습용 포맷으로 export한다. 산출물은 'Jsonl exporter validator'이며, 완료 기준은 'JSONL export 테스트 통과'이다.

이 Goal은 `JSONL export와 validation 구현` 범위를 완성형 기준으로 닫는 작업이다. 완료 판정은 `JSONL export 테스트 통과`이며, 구현 산출물만으로는 완료가 아니다. 단위테스트와 결과 문서가 함께 있어야 다음 Goal로 넘어갈 수 있다.

## 3. 작업 순서

1. `G5G-001` 선행 Goal의 결과 문서와 실패/미해결 리스크를 확인한다.
2. 작업 범위를 `JSONL export와 validation 구현`로 제한하고, 산출물을 `Jsonl exporter validator`로 고정한다.
...
7. 테스트 결과와 완료 기준 `JSONL export 테스트 통과` 충족 근거를 `docs/test-results/goals/G5G-002_jsonl_export_validation_unit_test_results.md`에 기록한다.
...
- `tests/G5G-002.JSONL.export.validation.Tests`
- `docs/test-results/goals/G5G-002_jsonl_export_validation_unit_test_results.md`

권장 테스트 위치:

- `tests/G5G-002.JSONL.export.validation.Tests/Program.cs`
...
- `Jsonl exporter validator` 산출물이 실제 public API, 모델, 문서, 테스트 중 어디에 속하는지 명확히 분리한다.
- 완료 기준은 `JSONL export 테스트 통과`이며, 이 기준을 테스트와 결과 문서에서 직접 증명한다.

추가 세부 지시:

- 산출물 `Jsonl exporter validator`이 어느 파일과 public API에 반영되는지 결과 문서에 적는다.
...
- Given `Jsonl exporter validator` 기본 입력, When Goal 범위의 기능을 실행하면, Then `JSONL export 테스트 통과`을 만족하는 결과가 나온다.
...
- CSV에 적힌 단위테스트 범위 `테스트는 'jsonl 스키마 테스트'를 직접 검증해야 한다. 정상 케이스, 실패/예외 케이스, 결정성이 필요한 경우 동일 입력 반복 케이스를 포함한다. 테스트 파일명과 테스트 명령을 결과 문서에 기록한다. Goal 범위 밖 동작을 검증하기 위해 새 구현을 끌어오지 않는다.`가 실제 테스트명 또는 assertion으로 추적 가능해야 한다.
...
- Goal ID와 제목: `G5G-002 JSONL export validation`
- 실행 일시와 실행 환경
- 수정/생성 파일 목록
...
- 실패 상세와 수정 여부
- 테스트하지 못한 항목과 이유
- 완료 기준 `JSONL export 테스트 통과` 충족 근거
- 다음 Goal 진행 가능 여부

## 10. 완료 판정 체크리스트

- [ ] 선행 Goal `G5G-001` 상태를 확인했다.
- [ ] 작업 범위 `JSONL export와 validation 구현` 밖의 변경을 하지 않았다.
- [ ] 원본 `DCGO/Assets/...` 파일을 수정하지 않았다.
- [ ] 대상 파일과 AS-IS 확인 파일을 결과 문서에 기록했다.
- [ ] 산출물 `Jsonl exporter validator`을 구현 또는 문서화했다.
- [ ] 단위테스트 `jsonl schema 테스트`를 작성했다.
- [ ] 단위테스트를 실행했고 실패가 없다.
- [ ] 금지 dependency 또는 금지 작업 위반이 없다.
- [ ] 결과 문서 `docs/test-results/goals/G5G-002_jsonl_export_validation_unit_test_results.md`를 작성했다.
- [ ] 완료 기준 `JSONL export 테스트 통과`을 결과 문서에서 증명했다.

## 11. 실행 프롬프트

```text
HeadlessDCGO.Engine Goal G5G-002를 수행하라.

반드시 먼저 이 상세 지시서를 읽어라:
docs/goal-specs/G5G-002_jsonl_export_validation.md

이번 작업은 G5G-002 하나만 완료하는 것이 목표다.
선행 Goal: G5G-001
작업 범위: JSONL export와 validation 구현
산출물: Jsonl exporter validator
단위테스트 범위: jsonl schema 테스트
결과 문서: docs/test-results/goals/G5G-002_jsonl_export_validation_unit_test_results.md
완료 기준: JSONL export 테스트 통과

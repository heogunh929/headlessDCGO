...
## §G. RD-R2C-ASSEMBLY 상환 (2026-07-20, 배치 7b — 옵션 A)
STOP `RD-R2C-ASSEMBLY`(비발화 실증) → **착지**. 미러 `SelectAssemblyClass`가 정적 feasibility 헬퍼에서 인터랙티브 pre-play 컴포넌트로 반전(instance state `playCard`/`selectedAssemblyCards` + `Select`, GManager 캐시)됐으므로, `GetPayingCostWithBaseCost`의 Assembly 감산 sub-region을 AS-IS(CardSource.cs:705-737) 기준 실이식: 라이브 pay 경로에서 `selectAssemblyClass.playCard == this && selectedAssemblyCards.Count == AssemblyConditionOf().elementCount`일 때 `Cost -= reduceCost`(DigiXros 감산과 동일 발화 구조, CardSource.cs:1203-1236). **적응**: AS-IS의 `if(checkAvailability) return 0;` 조기반환은 Assembly에서 **취하지 않음** — DigiXros와 달리 Assembly availability 투영은 열거 헬퍼 `Runtime/PlayCardAction.CreateAssemblyActionIfPlayable`(정밀 `base - reduceCost` 변형 제안)가 담당하므로 이 파이프가 checkAvailability에서 FULL base를 반환해야 함(0 반환 시 변형 붕괴). 라이브 로직 1:1, availability-투영 venue만 다름(아키텍처 치환·단순화 아님). 회귀: G9-065.Assembly 2 FAIL은 **선재**(base·change 동일 실증); 무신규회귀.

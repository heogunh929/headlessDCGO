...
## 총평 — 컷오버 표면 3가족
1. **삭제 2타이밍** ~~미러 Destroy()가 이미 호출 중이므로 조치 불요~~ — **C2 3차 시도가 오류 적발(2026-07-15 정정)**: 미러 `Destroy()`(인라인 인서트 :3490/:3502) 호출자는 **카드 2장뿐**(BT9_081/BT1_084). 보편 삭제 경로=①sink `ExecuteStagedDeleteAsync`(MatchStateMutationSink.cs:1203-1217, CardMoved 파생) ②상태기반 스윕(GameFlowProcessor:340-353→sink) ③배틀 `FinalizeAsync`(BattleResolver:201-214, 직접 MoveAsync) — 전부 브릿지-파생 의존이라 플립 시 삭제창 소실. **결정 #4(구속)**: sink DeleteKind pre-move에 배치당 `StackSkillInfos(OnDeletionHashtable(...), OnDestroyedAnyone/OnLeaveFieldAnyone)`(Destroy() :3490/:3502 의미론 미러, collect-before-removal)+배틀 FinalizeAsync 패자 이동에도 동일 transport ADAPTATION(배틀당 1배치)+기타 sink leave-경로(link/security/breeding-move) 감사 후 동일 처리+CardMoved 삭제-파생 제거. 배치 순차화는 AS-IS drain-위치(각 pick 후 RuleProcess→TriggeredSkillProcess)가 자연 대체 — D-1/D-2 witness로 검증, 잔존 co-drain 표면에만 SequenceByMinimumBatch 적용. payload의 battle/isDPZero 멤버는 sink cause 메타데이터에서 유도(불가 항목은 기록).

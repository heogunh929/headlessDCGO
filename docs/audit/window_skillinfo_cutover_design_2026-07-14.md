...
- **C1~C1d = 검수·커밋 완료(2026-07-15)**: C1(R-인서트 8+공급층 2, RD-C1-ATTACK-BGFX·RD-C1-CARDEFFECT-IDTHREAD 발견) → C1b(live ICardEffect 재-스레딩 3루틴+SwitchDefender async, RD-C1b-CARDARG) → C1c(카드 호출부 ~50곳 스윕, MIG3 id-격하 청산, dead 3종 식별) → C1d(RDW-04/02 emit 보강+변환, RDW-07 무수정 판명, RDW-01→BOUNCE-SNAPSHOT-TRANSPORT 재지정). 전 배치 296/132 byte-동일 파리티.
- **C2 1차 시도 = 정직 STOP(2026-07-15, 무수정·컴파일 유지)**: 아키텍처 봉쇄 적발 — **신 엔진 mid-window resume 미구축**(MultipleSkills 루프 위치가 C# 콜스택에 있어 choice-pause unwind에 소실; SkillWindowContinuation은 phase 커서·컷인 재귀 깊이 미보유; AgentSkillWindowChoicePort live 소비자 0 확인). 플립 강행 시 다중-효과 순서선택 창 전부 silent 파손. → **W1b 배치 신설**(승인 설계: resume=pass-head 재진입 — pass 루프가 상태-멱등[CanActivate 재유도·해소분 제거·답변 replay·in-flight는 OnceFlags 트랜잭션]이므로 외부화 필요분=phase 커서+미개시 타-phase 리스트+deepest-first 체인 재진입+TriggeredSkillProcess tail 소유권). 부수 지적: 플립 seam 전부에 AmbientMatchContext.Enter 필요·캐리어 전수 감사·검증은 다단계.

- **C2 2차 시도 = 무수정 STOP(검증-규모 사유) → 코디네이터 결정 3건으로 3차 재가동(2026-07-15)**: 2차가 산출한 레시피 채택 — ①캐리어 처분: 실질 double-fire 리스크는 공격 계열뿐(공급층이 R-타이밍 GAP-drop) → 공격 인라인 인서트 활성+공격 emit 제거, 나머지 캐리어 제거는 정리 성격 ②pause 신호는 ChoiceController 요청으로 자동 보존, resume=RecordAnswer+ResumeSuspendedWindowsAsync. **결정**: (1) seam-5 OnKnockOut(발명 타이밍)은 배틀 패자 삭제가 미러 Destroy() 도달 시 은퇴, 미도달 시 기제만 교체+ADAPTATION 기록 — 감사-후-분기 (2) A3 입도=batch-group마다 stack→AutoProcessCheck 1회(피더 순차화, 루프 불변) (3) C2 DONE 기준=플립+우선 witness green/분류+전체 fail-set 델타 4분류(하네스 phase-triage 장기 꼬리는 C2b 소관, 검증-규모는 STOP 사유 아님 — green 계수는 게이트가 아니라 분류가 게이트).

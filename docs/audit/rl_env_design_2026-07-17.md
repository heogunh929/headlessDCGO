...
- **RD-RLENV-05** **[상환·재해석 — 배치 7b(옵션 A)]**: `SpecialPlay` 펌프 분기 제외는 **STOP이 아니라 옵션 A의 의도적 설계**로 확정. DigiXros/Assembly 컴포넌트 클러스터(`SelectDigiXrosClass.Select`/`SelectAssemblyClass.Select`)는 완전 포팅(RD-P6C1-5/RD-R5-04 상환: `Permanent.CanSubstituteForDigiXrosCondition`+`SelectHandEffect` 미러 실착지). 그 소비는 **일반 PlayCard 경로**로 표면됨 — HasDigiXros/HasAssembly 카드가 `PlayCardAction.GetLegalActions`로 이미 제안(availability cost-projection)되고, 펌프 플레이가 `Cec.PlayCardAction → PlayCardClass.PlayCard → SelectDigiXros/SelectAssembly`(`CardController.cs:3387-3404`)로 라우팅되어 소재 tuck + 라이브 코스트 감산(`CardSource.cs:1173-1233`) 적용. 따라서 SpecialPlay는 펌프 표에 **복귀하지 않으며**(별도 패킷은 동일 플레이의 중복 발명), RL 관점에서 해당 카드는 플레이 가능(일반 PlayCard 레인). 비-펌프 SpecialPlayAction 레인(G8-006 핀)은 불변. 실증: `tests/RD-BATCH7B.Witness.Tests`(펌프 DigiXros 빈-프레임 플레이 완주, 트래시 소재 tuck·코스트 6→3).

...
|---|---|---|
| RD-P6C2-1 | `AscensionProcess` | `CardObjectController` (the AS-IS static zone-move helper class) doesn't exist on the mirror at all. (R2-B) Gap NARROWED: the `CanAddSecurity` gate + Yes/No `SetBoolSelection`/`WaitForEndSelect`/`SelectedBoolValue` selection all have mirror substrate now — only `CardObjectController.AddSecurityCard` (RemoveFromAllArea + DigiEgg/Token branches + Insert-security-top + IAddSecurity) remains, a CardController-region zone-move re-housing out of R2-B keyword scope. Still STOP. |
| RD-P6C2-2 | `RetaliationProcess` | ~~`DestroyPermanentsClass`~~ — **RESOLVED (R2-B)**: AS-IS's terminal `new DestroyPermanentsClass(targets, hashtable).Destroy()` is the same deletion pipeline the mirror exposes through `DeletePeremanentAndProcessAccordingToResult` (AS-IS's own wrapper of DestroyPermanentsClass, CardEffectCommons.cs:463) — terminal batch-delete issued via that verified substrate with null continuation. Full winner/loser/tie scan ported 1:1. |
| RD-P6C2-3 | `CanActivateDecoy`, `CanActivateFragment` | ~~`Permanent.CanBeDestroyedBySkill`~~ — **RESOLVED (R2-B)**: R1-d ported `Permanent.CanBeDestroyedBySkill` (Permanent.cs:3035, AS-IS Permanent.cs:3309-3365); both CanActivate bodies ported verbatim. |
...
| RD-P6C2-6 | `PartitionProcess` | ~~`PartitionClass`~~ — **RESOLVED (R2-B)**: `PartitionClass` (per-group SelectCardEffect + `PlayPermanentCards` free play) ported 1:1 — both substrate pieces already exercised by Fragment/Fortitude; CreateDebuffEffect + Play-Log stripped as UI; `Color`/`Color2` already `string` on the mirror `PartitionCondition`. |

...
| G10 mass `MassDeDigivolveThenConditionalDestroyEffect` | **✅ 상환**(2026-07-08) | AS-IS 두 독립 스캔 구조로 재작성: 파라미터를 `deDigivolveTarget`(디지그레이드 루프, DP무관) + `destroyTarget`(destroy 재스캔, DP≤5000 등 전체술어) **분리**. 단일 `_target`+`_destroyIf` 폴딩 제거 → 배선이 DP를 잘못된 술어에 넣는 foot-gun 구조적 차단. 테스트가 split 명시검증(고DP도 디지그레이드됨+destroy 제외). 회귀 green. | 상환 |
| G12 `ChooseCountThenTrashDigivolutionEffect` | **✅ 상환**(2026-07-08) | ① 자격필터: raw `Count>0` → `HasTrashableDigivolutionCards`(ST2_03/09와 동일, `Count(!보호)≥1`). ② count min: `canNoSelect` 파라미터 추가, `minCount = canNoSelect?0:1`(CanNoSelect:false→최소1). ③ 트래시 스킵: `RemoveSourcesAsync`가 **Trash 목적지**에서 보호 소스(`TrashProtectedKey`) 스킵(AS-IS `ITrashDigivolutionCards` 미러; window 위치수집 후 보호 필터 — 깊이 안 파고듦). ST2_03/06/09 트래시도 함께 충실화. 테스트 4건, 회귀 344 green. **잔여**: per-card 보호 **부여**(`CanNotTrashFromDigivolutionCardsClass`)는 여전히 stub → 스탬프하는 카드 생기면 자동 활성(아래 별도 debt). | 상환 |

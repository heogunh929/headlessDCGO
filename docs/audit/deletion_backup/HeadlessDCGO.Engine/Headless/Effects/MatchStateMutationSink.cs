...
// 미러 원가(재이관 대상): Script/CardController.cs::DestroyPermanentsClass · HandBounceClaass · DeckBottomBounceClass · DeckTopBounceClass · IPutSecurityPermanent · IReduceSecurity · CardObjectController.AddHandCards; Permanen
...
public sealed class MatchStateMutationSink : IEffectMutationSink
{
    ...
    private void ApplyCore(EffectMutation mutation)
    {
        ...
        if (_context is { } immunityCtx
            && new Assets.Scripts.Script.CardEffectCommons.Permanent(immunityCtx, targetId, record.OwnerId)
                .TopCard.CanNotBeAffected(HeadlessDCGO.Engine.Headless.Bridge.BareCauseEffect.ForOrNull(immunityCtx, mutation.SourceEntityId)))
        {
            _skipped.Add(mutation);
    ...
    private bool IsPlayerRestricted(HeadlessPlayerId player, string restrictionKey, HeadlessEntityId causingSourceId)
    {
        ...
        // AS-IS Player.CanAddSecurity/CanAddMemory thread the causing effect (null for a rule-sourced add) into
        // cannotAdd*'s CardEffectCondition; ForOrNull reproduces the AS-IS null cause instead of a source-less stub.
        Assets.Scripts.Script.CardEffectCommons.ICardEffect? cause =
            HeadlessDCGO.Engine.Headless.Bridge.BareCauseEffect.ForOrNull(_context, causingSourceId);
    ...
    private void ApplyDigivolutionSourceRemoval(EffectMutation mutation, HeadlessEntityId hostId, ChoiceZone? returnToZone)
    {
        ...
        if (returnToZone is ChoiceZone destination)
        {
            ...
            if (_context is { } returnImmuneCtx
                && new Assets.Scripts.Script.CardEffectCommons.Permanent(returnImmuneCtx, hostId)
                    .TopCard.CanNotBeAffected(HeadlessDCGO.Engine.Headless.Bridge.BareCauseEffect.ForOrNull(returnImmuneCtx, mutation.SourceEntityId)))
            {
                _skipped.Add(mutation);
        ...
        else
        {
            ...
            if (_context is { } stackTrashCtx
                    && ((HeadlessDCGO.Engine.Headless.Bridge.BareCauseEffect.ForOrNull(stackTrashCtx, mutation.SourceEntityId) is { } stackTrashCause
                            && new Assets.Scripts.Script.CardEffectCommons.Permanent(stackTrashCtx, hostId)
                                .ImmuneFromStackTrashing(stackTrashCause))
                        || new Assets.Scripts.Script.CardEffectCommons.Permanent(stackTrashCtx, hostId)
                            .TopCard.CanNotBeAffected(HeadlessDCGO.Engine.Headless.Bridge.BareCauseEffect.ForOrNull(stackTrashCtx, mutation.SourceEntityId))))
            {
                _skipped.Add(mutation);
                _applied.Add(new AppliedMutation(mutation.Kind, hostId, "restricted"));
                return;

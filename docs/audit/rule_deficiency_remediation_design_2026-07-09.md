...
**Phase 3b(다음) = 실제 라이브 컷오버**: `AutoProcessAsync`를 `DriveAsync`+`WindowResolutionController`로 교체. 필요: ①실 `IWindowChoicePort` — **key-based record-replay**(위 P1-a): ChoiceController에 order/optional 창 열고 미답이면 `WindowChoicePendingException` throw, resume 시 effect-id 키로 resolved 답 판독(호출 순번 아님). ②`ResolveChoiceAsync`(MetadataActionProcessor:599)에 `WindowResolutionController.Pending` resume 분기 추가(DeferredActivations 분기 옆) ③Collect 통합(scheduler batch + activated bridge 3 timing set을 단일 seed로) ④**pick-간 RuleProcess(F3)**: AS-IS는 매 pick 後 RuleProcess()(MultipleSkills:398-403) — 배선의 `DrainNewTriggers`/`Gate` delegate가 post-rule-process 보드를 보게 하거나 루프에 명시 seam 추가. **주의: skip/decline/fizzle `continue` 경로는 현재 DrainNewTriggers 미호출** — AS-IS가 그 경우에도 RuleProcess 도는지 확인해 배선에서 균일 적용 필요. ⑤once-use 소모 지점 P1-b 확인(cost-suspend 시) ⑥테스트 AS-IS 갱신(G2F-002/G1F-004 고정순서→player-choice, G11-004 소모시점).

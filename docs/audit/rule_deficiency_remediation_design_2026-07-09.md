...
### 📍 Phase 3a 진행 (2026-07-10 후속 세션, branch=stage5-phase3-windowresolver) — resume 인프라 구축(무배선)
Phase 3 컷오버의 최난제 F6(suspend/resume 지속)를 라이브 컷오버 前에 격리 구축·적대검수. **Phase 3a-i**: `WindowResolver` 루프를 C# 컷인-재귀 → **명시적 프레임-스택**으로 변환(동작 완전 동일; C# 콜스택이 agent-choice pause를 넘어 보존 안 되므로 중첩 창 상태를 데이터로 보유). **Phase 3a-ii**: `WindowContinuation`(persist 대상={프레임-스택, in-flight pick}) + `DriveAsync`(fresh/resume 통합 진입) + `WindowChoicePendingException`(기존 DeferredChoicePendingException 미러) + `WindowResolutionController`(DeferredActivationController 미러, match-resettable). resume 모델: **body-suspend**=commit 후 body만 replay(재-pick/재-commit 없음, once-use 1회), **choice-suspend**=mutation 0, port가 기록된 답 재생, **nested 컷인**=프레임-스택 전체 persist→cut-in-first(RD-17) 순서 재개 유지. 단위테스트 `tests/Stage5-WindowResume.Tests`(37검: body/multi-suspend/choice/nested/controller + keyed-replay). 회귀 **389/389**·기존 창 테스트 전부 green(무배선이라 라이브 무영향).
**⚔️ 적대 검수 결과(독립 reviewer, 2026-07-10)**: **P0 없음**(프레임-스택 변환·resume 배선 내부 일관, 6 hunt 전부 "no bug" 검증 — cut-in 재drain·ChainLimit off-by-one·choice-unwind half-done·양 suspend 상호배타·IsExhausted·sync-window 무영향). 상환:
  - **P1-a(테스트 갭 상환)**: choice-suspend resume는 pass 전체 재실행(AS-IS in-place 재개와 구조 상이하나 DeferredChoiceProvider 재생과 동일 계약=정당 substrate). **핵심 계약 발견**: Phase-3b 실 port는 반드시 **key-based record-replay**(order=정렬 대상 effect-id 집합, confirm=effect-id로 키)여야 함 — pick이 resolve되면 그 choice-point가 이후 pass에서 사라지므로 **positional(호출 순번) 재생은 오정렬**. 상환=`KeyedReplayPort` 테스트 추가(multi-choice-per-pass 재생 멱등, order가 A resolve 후 사라짐 검증).
  - **P1-b(Phase-3b 노트)**: once-use는 commit(body 前) 소모인데 AS-IS `OnProcessCallbuck`은 `Activate_Effect`(cost) **後**(ICardEffect.cs:1120 vs 1278). 3a가 바꾼 것 아닌 기존 F5/RD-12 결정이나, **cost 지불 자체가 suspend하면** pause 동안 헤드리스는 once-flag 소모済·AS-IS는 미소모(net 1회는 동일). Phase-3b에서 cost-suspend 카드 배선 시 소모 지점 재확인 필요.
  - **P2**: resume ResolveBody 前 `cancellationToken` 체크 추가(상환); `DrainCutInInto`의 `frames.Count>0` 가드는 도달불가-false 방어코드(무해, 유지).
### 📍 Phase 3b-i 완료 (2026-07-10, 커밋 61652211) — 실 ChoicePort + WindowChoice resume 심 (메인 루프 미구동)
라이브 컷오버(3b-iii)의 기반. **`AgentWindowChoicePort`**(key-based record-replay 실 port: 첫 조우 시 `ChoiceType.WindowChoice` 창을 ChoiceController에 열고 `WindowChoicePendingException` throw, 재-drive마다 기록된 답 재생 — effect-id 키라 resolve된 pick의 choice-point 소멸해도 정합; AS-IS OpenSelectCardPanel(_MaxCount:1)+Activate_Optional 미러). **`WindowResolutionController`**에 기록-답 저장소 추가(pause 넘어 persist). **EngineContext.WindowResolution** 배선(DeferredActivations 미러). **ChoiceType.WindowChoice** 추가. **`ResolveChoiceAsync` WindowChoice 분기**(답 기록→resolve→`BuildMainLoopDeps`로 continuation 재-drive→완료 시 clear, 추가 창 시 재-suspend). **`WindowResolverWiring.BuildMainLoopDeps`**(실 port+공유 cut-in drain, `DrainSchedulerCutIns` 추출). 통합테스트 `tests/Stage5-WindowChoiceResume`(16검, 실 ChoiceController+실제 프로세서 관통): order pick·optional yes·optional no. 회귀 **390/390**(ChoiceType 스키마 count 16→17). **주의: AS-IS optional 확인 위치=body 내부(Activate_Optional), 헤드리스=pre-body ConfirmOptionalAsync — 기존 substrate 결정(RD-13), confirm→consume→body 순서는 AS-IS(ICardEffect Activate_Optional→OnProcessCallbuck→Activate) 일치 확인.**

### 📍 Phase 3b-ii 완료 (2026-07-10) — dispatching ResolveBody + activated-bridge 마커 합성 (무배선)
라이브 컷오버(3b-iii)의 활성효과-경로 절반을 격리 구축. **① 카테고리 세트 단일화**: `GameFlowProcessor`의 4개 private 세트(SubjectScoped/Boundary/EventBroadcast/OncePerTurn)를 공유 `Effects/ActivatedBridgeTimings.cs`로 이동(배치 브릿지와 창-배선이 동일 분류 참조, 발산 방지 — 순수 이동, 회귀·RuleAudit로 등가 확인). **② 마커 표현**: activated 트리거를 마커-보유 `TimingWindowTrigger`로 표현(`WindowResolverWiring.ActivatedBridgeKey`+category+drivingEvent를 resolve-context Values에; card=SourceEntityId, timing=Request.Timing, 합성 EffectId=`activatedBridge:{card}:{timing}:{seq}`). **③ dispatching Gate/Commit/ResolveBody**: 마커면 Gate=true(OnUnTappedAnyone만 synthetic-key `{card}:bridgeOnce:{timing}` OnceFlag CanActivate/Consume 미러), ResolveBody=`ActivatedEffectResolver.ResolveAsync`를 `try/catch(DeferredChoicePendingException)→DeferredActivations.Suspend+Suspended`로 정규화(배치 GameFlowProcessor:748-761 미러); 비-마커는 기존 scheduler. **④ 합성** `CollectActivatedBridgeTriggers`: 배치 `BridgeActivatedTriggersAsync` 스캔(subject dedup / boundary 전-배틀존 dedup / broadcast per-event) 1:1 미러, OnceFlag만 up-front 소모 대신 window Gate/Commit로 이동. 테스트 `tests/Stage5-ActivatedBridge`(14검: dispatch→resolver+suspend 정규화(실 EX8_074), 비-마커→scheduler, OnUnTapped 캡, subject/boundary 합성). 회귀 **391/391**·RuleAudit 0. **무배선**(AutoProcessAsync 본문 불변) — 마커 resume의 이중경로(창 InFlightPick vs DeferredActivations.Pending) 조율은 3b-iii.

### 📍 Phase 3b-iii 완료 (2026-07-10, 커밋 77e6f4a2) — AutoProcessAsync 라이브 컷오버 (창-루프 메인)
배치 파이프라인(collect[gate/cap]→EnqueueOrdered[고정순서]→ResolveAll→BridgeActivated→OptionalPromptQueue)을 AS-IS MultipleSkills 미러 재진입 창-루프로 **실교체**. **① 통합 seed** `CollectUnifiedSeed`: scheduler(gate/cap 제거→창) + activated 마커. **OnDeletion만 collect서 배치-collapse**(1 엔트리=첫 gate-pass subject, on-fire 미러) — AS-IS 단일 StackSkillInfo, 아니면 동일효과 N카피가 order-choice로 오분기(회귀서 실측·수정). **② `BuildLiveMainLoopDeps`**: GateLive(end-game 단락)+CommitLive+ResolveBodyLive(dispatch + one-shot fire-then-clear[body 後] + F3 pick-간 `RuleProcessAsync`)+통합 cut-in drain. **③ HasEffectsAt 필터**(신규): activated 마커는 해당 timing에 실제 반응 카드만 — 배치는 순차 no-op 허용했으나 창에선 no-op 마커가 실효과와 order-choice **경쟁**(스퓨리어스, 회귀서 실측·수정). **④ SuspendedExternally**: activated body suspend는 창 InFlightPick 아닌 DeferredActivations 재개; 액션프로세서가 활성 완료 後 창 재-drive(잔여 스택). 창 order/optional은 WindowChoice(3b-i). **⑤ 테스트 AS-IS 갱신**: D3/OPT2 고정순서·OptionalPromptQueue→player order-choice + inline optional confirm(RD-13/14/15); Stage5-ActivatedBridge collect는 HasEffectsAt 반영(BT2_034 실반응 vs inert). `EnrichWithEventSubject`/`ReclassifyKind`/`RuleProcessAsync` internal화. 회귀 **391/391** · RuleAudit 0(20게임 3680스텝). **적대검수 진행중.** 잔여 dead-path(BridgeActivatedTriggersAsync/EnqueueOrdered/QueueOptionalPrompts/RequestNextOptionalPrompt/MandatoryEffectOrdering/OptionalPromptQueue) 정리.

**Phase 3b(남은) = 실제 라이브 컷오버**: `AutoProcessAsync`를 `DriveAsync`+`WindowResolutionController`로 교체. 남은 작업:
- ~~**3b-ii = Collect 통합 + dispatching ResolveBody**~~ ✅**완료(위 §Phase 3b-ii)** — 조사·구축·테스트 완. 원설계:
  - **통합 seed**: (A) scheduler = `AutoProcessingTriggerCollector.CollectAllTriggers`(EffectRegistry의 IHeadlessCardEffect, EffectId dedup) + AutoProcessAsync의 per-event `seen`·`firedDeletionEffects`(deletion-batch)·disable게이트·enrich·ReclassifyKind (단 **collect-time CanResolve gate/OnceFlags.TryActivate 소모 제거**→window Gate/Commit). (B) activated = **card-scan 신규 합성**(CollectAllTriggers는 activated 미포함): 3 timing set을 `GameFlowProcessor.cs:682-724` 로직대로 스캔해 `(card,timing,drivingEvent)` 튜플 → **마커 실은 TimingWindowTrigger**(EffectContext.Values에 `activatedBridge=true`+card(SourceEntityId)+timing(Request.Timing)+drivingEvent 메타; 선례 `DelayedOneShotKey`).
  - **dispatching ResolveBody**: 마커 있으면 `ActivatedEffectResolver.ResolveAsync(ctx,card,owner,timing,ct,drivingEvent)`를 **try/catch(DeferredChoicePendingException)** 로 감싸 catch 시 `DeferredActivations.Suspend(card,timing,owner,drivingEvent)`+return Suspended; 없으면 기존 scheduler(Enqueue+ResolveAllAsync, IsSuspended 판독).
  - **Gate 마커 분기**: activated 마커는 `EffectRegistry.Find(EffectId)` null이라 현 Gate가 false로 **드롭함** → 마커면 gate=true(게이팅은 ResolveAsync 내부: uniform case가 CanResolve+MaxCountPerTurn 자체 체크), **단 OnUnTappedAnyone만** caller-cap = synthetic key `"{card}:bridgeOnce:{timing}"` OnceFlag(GameFlowProcessor:737-746)를 Gate(CanActivate)+Commit(Consume)에 미러.
  - **캡 모델(카테고리별, 마커에 category 실어 분기)**: subject-scoped=per-event 자연(seen (subject,timing)), boundary=per-turn 자연(플래그 없음, 매턴 1이벤트), event-broadcast=per-event(dedup 없음)+effect MaxCountPerTurn self-cap(resolver uniform 내부 Consume). event-broadcast는 drivingEvent별 별 트리거.
  - **테스트**: 마커 트리거가 ActivatedEffectResolver로 라우팅(실 카드 activated effect 적용 확인), scheduler 트리거는 스케줄러로, DeferredChoicePendingException→Suspended 정규화. Place 헬퍼는 `tests/G9-013.DeferredActivationResume` 참조.
- **3b-iii = 컷오버 + F3 + 적대검수**: AutoProcessAsync 본문 교체. ①**pick-간 RuleProcess(F3)**: AS-IS 매 pick 後 RuleProcess()(MultipleSkills:398-403) — 배선 `DrainNewTriggers`/`Gate`가 post-rule-process 보드를 보게(주의: skip/decline/fizzle `continue` 경로도 AS-IS RuleProcess 도는지 확인) ②**body-suspend within window**: 창 body가 스케줄러/activated로 suspend 시 pending choice는 body-choice 타입 → 기존 body-resume(RevealSelect 등)+DeferredActivations resume과 window 재-drive 조율(WindowResolution.Pending InFlightPick 재개) ③once-use 소모 collect→commit 이동(P1-b) ④MandatoryEffectOrdering/OptionalPromptQueue dead-path 정리 ⑤테스트 AS-IS 갱신(G2F-002/G1F-004 고정순서→player-choice, G11-004 소모시점). **적대 검수 필수(컷오버 前).**
...
**컷오버 계획(커밋 단위)**:
- **P0-리스크(先-확정)**: OnceFlag 소모 **수집→commit 이동**. **[정정 2026-07-10 적대검수 F5]** AS-IS `Activate_Execute`(ICardEffect.cs:1118-1124)는 `OnProcessCallbuck`(=RegisterUseEffectThisTurn)을 `UseOptional||!IsOptional` 통과 직후 **body 실행 前(commit 시점)** 발화하고 `SetOnProcessCallbuck(null)`로 재실행 1회 보장 — 즉 "해소-성공 後"가 아니라 "accept/mandatory 확정+CanActivate 재확인(MultipleSkills:366) 後, body 前". 따라서 body가 suspend/soft-fizzle해도 소모 유지(환불 아님). 유일하게 shadow-불일치이며 라이브 동작(G11-004·G3.5-F4 수집-소모 pin)·optional-decline·commit-gate fizzle(RD10) 재작성. WindowResolver는 commit(body 前 소모)+commit-gate 재확인으로 구현. **suspend 시 재실행이 재-commit 안 하도록 idempotent(Phase 2 WindowResolutionController가 in-flight pick 소유).**

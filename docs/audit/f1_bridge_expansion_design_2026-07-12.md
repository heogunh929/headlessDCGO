...
2. **M1 — Tier 1 확산**(타이밍별 골): ~~OnLoseSecurity~~ ✅(413) → ~~OnMove~~ **✅(414, 2026-07-12)** → ~~OnDiscard*~~ **✅(415, 2026-07-12)** → ~~OnAddHand/OnAddSecurity~~ **✅(416, 2026-07-12)**. **Tier 1 완료.** 각 witness 2~3장 + 적대 리뷰.
   ...
   - **M1=OnAddHand/OnAddSecurity 완료(416, 2026-07-12)**: 2 타이밍 EventBroadcast 등록. **AS-IS 재도출이 두 타이밍의 근본적 batch 비대칭 확정**: OnAddHand=`AddHandCards(list, isDraw, cardEffect)` 1콜=1 StackSkillInfos over 전체 리스트 → **effect-batch(collapse 필요)**; OnAddSecurity=`IAddSecurity.AddSecurity`가 단일카드마다 발화(AddSecurityCard가 useEffect와 무관하게 매 카드 IAddSecurity 실행) → **per-card(collapse 금지)**. cause: OnAddHand 게이트는 `CardEffect != null` 요구(턴 드로우 cardEffect=null→미발화)이나 **AS-IS엔 두 게이트 변종**(`CanTriggerOnHandAdded`=cause 내장강제, `CanTriggerWhenAddHand`=cause 미강제·카드가 `cardEffect=>cardEffect!=null` 명시공급); 실카드(BT9_021/EX4_022/BT15_083)는 후자에 명시 cause 공급 → 헤드리스 게이트를 `Func<CardSource?,bool>`(nullable cause)로 1:1 미러(스캐폴드 버그 정정: 기존 게이트가 reactor 자신의 SourceEntityId를 cause로 오독+cause 미강제). OnAddSecurity=`CanTriggerWhenAddSecurity`→lose-security shape(player-scope, no cause) 그대로. substrate 재사용: `AddHandBatchIdKey`+`AddHandCauseEffectIdKey`(OnDiscard `DiscardBatchId`/`DiscardCauseEffectId` 패턴, 신규 키·공유 카운터 `NextAddHandBatchId`), sink `ResolveAddHandBatchId`(per-flush)+ApplyDraw/ReturnToHand 스탬프, `IZoneMover.Draw/AddToHandAsync`에 batch·cause optional 파라미터 추가(기존 호출부 6곳 named-ct 정정). emit=TriggerTimingMap이 이미 OnAddToHand/OnAddToSecurity 파생(M0). collapse `firedAddHandBatch`(card,id)=OnAddHand만. witness BT15_083(self OnAddHand, cause=owner+IsDigimonEffect, SuspendSelfAndGainMemoryBody)·BT8_090(self OnAddSecurity, no cause, 동형) + uncapped 픽스처 TfxOnAddCounter(OnAddHand collapse+2 independent+cause-gate+player-scope, OnAddSecurity per-card +N). **자체 적발·상환**: co-drained OnAddSecurity per-card triggers가 BatchId 부재로 단일 spurious 크로스-fire ORDER CHOICE에 충돌→미응답 drop(+0)—AS-IS는 순차해소라 order 미요구. 각 트리거 BatchId=`drivingEvent.Sequence`(단조 per-event) 스탬프로 오름차순 순차화(OnDiscard 독립배치 선례와 동형)→order prompt 제거. **이연**: (1) F1-M1-INHERITSCAN—BT9_021(OnAddHand)·BT9_003(OnAddSecurity) inherited(진화원) 리액터 latent(witness는 non-inherited top-permanent). (2) **F1-ADDHAND-REVEAL**—AS-IS RevealLibrary는 select/remainder를 hand로 add(cause 有)해 OnAddHand 발화하나, 헤드리스 RevealAndSelect.MoveAsync가 reveal source id 미스레드→cause 없어 under-fire(safe 방향, over-fire 아님; 실카드 의존 0). EX4_022(opponent-scope bounce)는 select-bounce body 조립 비용으로 미포팅(opponent player-scope는 픽스처 negative로 커버). scheduler-half(BroadcastTimings) 미갱신(전 리액터 uniform ActivateClass). 회귀 416/416·RuleAudit 0.

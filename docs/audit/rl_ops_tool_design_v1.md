...
| M5 ✅ | SDK 정리 + 참조 러너 공개품질 + OpenClaw 스킬 · TCP 브리지(`--listen`) | **통과 2026-07-31 [실측]**: TCP 루프백 학습 1런 완주(워커 2 = 상주 호스트 9310/9311, 포트+rank 규약, SubprocVec) + 호스트 TCP 3세션 연속 + SDK(`dcgo_arena`: ArenaClient/RandomPolicy/LLMPolicy/state_to_text) 래더 한 판 완주. 구현 노트: ① 호스트 `--listen`=순차 세션(끊기면 다음 accept) ② bridge tcp://는 블로킹 소켓 필수(makefile+settimeout 조합이 재접속 행 — 실측) + close는 읽기 makefile까지(안 닫으면 FIN 미발신→호스트 영구 대기) ③ LLM 정책은 OpenAI 호환 규격·파싱 폴백까지 구현, 실 endpoint 실측은 대기 ④ OpenClaw 스킬 = skills/dcgo-arena/SKILL.md ⑤ runner do_POST 지역변수가 config()를 가리던 결함 수리 |

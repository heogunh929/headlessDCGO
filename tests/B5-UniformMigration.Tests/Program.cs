...
var tests = new (string Name, Func<Task> Body)[]
{
    ...
    ("(RDx-A3 split) collect gate = canUse half only; per-pass gate = canActivate half only; cap gates both", SplitGateHalves),
};
...
async Task OptionalSelectDeclineAndAccept()
{
    ...
    var effectId = new HeadlessEntityId($"{self.Value}:ae");   // unhashed cap partition = per card (AS-IS IsSameEffect)
...
async Task SplitGateHalves()
{
    // AS-IS: the collect filter is CanTrigger (cap + CanUseCondition, ICardEffect.cs:319-358), evaluated ONCE at
    // GetSkillInfos; the window loop re-checks ONLY CanActivate (cap + CanActivateCondition, :364-457) on stacked
    // skills (MultipleSkills.cs:122/164-165) — never CanTrigger. So CanCollectAt must see the canUse half but NOT
    // the canActivate half, and CanActivateAt the reverse; the once-per-turn cap lives in BOTH AS-IS gates.
    EngineContext context = EngineContext.CreateDefault(randomSeed: 87);
    context.TurnController.Initialize(new[] { P1, P2 }, P1);
    var cards = (CardDatabase)context.CardRepository;
    cards.Upsert(new CardRecord(new HeadlessEntityId("TfxSplitGateDrawdef"), "TfxSplitGateDraw", "SG",
        new Dictionary<string, object?>(StringComparer.Ordinal), CardType: "Digimon"));
    var self = new HeadlessEntityId("1:battle:SG");
    void SetGates(bool use, bool act) => context.CardInstanceRepository.Upsert(new CardInstanceRecord(
        self, new HeadlessEntityId("TfxSplitGateDrawdef"), P1,
        Metadata: new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            [HeadlessDCGO.Engine.Assets.Scripts.CardEffect.TestFixtures.TfxSplitGateDraw.GateUseKey] = use,
            [HeadlessDCGO.Engine.Assets.Scripts.CardEffect.TestFixtures.TfxSplitGateDraw.GateActKey] = act,
        }));
    SetGates(use: true, act: true);
    await context.ZoneMover.MoveAsync(new ZoneMoveRequest(P1, self, ChoiceZone.None, ChoiceZone.BattleArea));
    const EffectTiming T = EffectTiming.OnEnterFieldAnyone;

    // canUse FALSE, canActivate TRUE: NOT collected (AS-IS CanTrigger filters it out of the stack), but a
    // hypothetically-stacked marker WOULD pass the per-pass gate (only the canActivate half re-checks — ST4_14).
    SetGates(use: false, act: true);
    AssertTrue(!ActivatedEffectResolver.CanCollectAt(context, self, P1, T), "canUse=false: collect gate rejects");
    AssertTrue(ActivatedEffectResolver.CanActivateAt(context, self, P1, T), "canUse=false: per-pass gate still passes (canUse never re-checks)");

    // canUse TRUE, canActivate FALSE: collected (CanTrigger holds), but per-pass gate-false (deferred, re-tests).
    SetGates(use: true, act: false);
    AssertTrue(ActivatedEffectResolver.CanCollectAt(context, self, P1, T), "canActivate=false: collect gate still passes");
    AssertTrue(!ActivatedEffectResolver.CanActivateAt(context, self, P1, T), "canActivate=false: per-pass gate rejects");

    // Cap consumed: BOTH gates reject (AS-IS cap check lives in CanTrigger :339 AND CanActivate :366-372).
    SetGates(use: true, act: true);
    var effectId = new HeadlessEntityId($"{self.Value}:ae");   // unhashed cap partition = per card
    var request = new EffectRequest(effectId, P1, EffectTimings.ToTriggerName(T), new EffectContext(P1, self));
    context.OnceFlags.Consume(request, maxCountPerTurn: 1);
    AssertTrue(!ActivatedEffectResolver.CanCollectAt(context, self, P1, T), "capped out: collect gate rejects");
    AssertTrue(!ActivatedEffectResolver.CanActivateAt(context, self, P1, T), "capped out: per-pass gate rejects");
}

...
// Drive the pump's natural Active->Draw->Breeding->Main auto-flow to the turn player's main wait; the OLD
...
async Task DeclareTargetAttackAsync(DcgoMatch match)
{
    ...
    // Drive the pump segment released by the action through the attack pipeline to quiescence; a single
    // StepAsync under-drives battle resolution on the pump surface (4b B3 RL re-aim, OLD cadence retired).
    for (int i = 0; i < 16 && !match.HasPendingChoice(); i++)
    {
        using AmbientMatchContext.Scope _ = AmbientMatchContext.Enter(match.Context);
        await match.StepAsync();
    }

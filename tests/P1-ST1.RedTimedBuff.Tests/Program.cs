...
// Phase 1 — ST1 Red timed-buff wave: ST1_13 (Option).
//   [Main]     select 1 of your Digimon -> +3000 DP until end of turn
//   [Security] all your Digimon -> Security Attack +1 until your next turn end
// The buff is a duration-tagged continuous binding the gate folds in and EffectDurationExpiry removes.
...
async Task ST1_13_SecuritySaBuff()
{
    (EngineContext context, HeadlessEntityId mine) = await OwnDigimon();
    context.TurnController.Initialize(new[] { P1, P2 }, P1);
...
async Task ST1_14_SecurityZoneScope()
{
...
async Task ST1_08_DigivolveBuff()
{
    (EngineContext context, HeadlessEntityId mine) = await OwnDigimon();
    context.TurnController.Initialize(new[] { P1, P2 }, P1);

    // Activated/choice effects are skipped by auto-registration (resolved via the activation flow).
...
// Owner P1 has one Digimon in Security and one in the battle area.
async Task<(EngineContext, HeadlessEntityId, HeadlessEntityId)> SecurityAndBattleDigimon()
{
...
async Task<(EngineContext, HeadlessEntityId)> OwnDigimon()
{
...
ICardEffect Effect(CEntity_Effect card, EffectTiming timing, EngineContext context)
{

---
description: Port one Digimon card 1:1 from DCGO/ into the headless engine, driven by the card's pre-extracted brief (pipeline-v2). Reads porting/briefs/<SET>.<COLOR>/<ID>.md instead of injecting the 90KB catalog.
agent: porter
---
...
- 쓰기는 화이트리스트(카드 폴더 · `porting/stop/`)만. `tests/` 안 씀(자동바인딩 테스트는 강모델 소유). `DCGO/` 읽기 전용, 커밋 금지.

## 절차
1. **브리프 확보.** `porting/briefs/<SET>.<COLOR>/$ARGUMENTS.md` 읽기(id에서 SET/COLOR 판별). 없으면 먼저 생성: `python3 porting/scripts/make-card-brief.py <SET> <COLOR> $ARGUMENTS`.
...
6. **STOP 기록.** 실제로 STOP 한 분기가 있을 때**만** `porting/stop/<SET>.<COLOR>.md` 에 `$ARGUMENTS | 이유 | 원본심볼` append. STOP 이 없으면 stop 로그에 아무것도 쓰지 않는다.
...
- 이 카드의 브리프: `porting/briefs/<SET>.<COLOR>/$ARGUMENTS.md`  ← **이것이 유일한 조회원천. 여기 있는 심볼은 STOP 금지.**
